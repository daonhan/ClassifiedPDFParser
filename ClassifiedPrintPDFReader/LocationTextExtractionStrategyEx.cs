...
namespace ClassifiedPrintPDFReader
{
    ...
    {
        private List<ExtendedTextChunk> m_DocChunks = new List<ExtendedTextChunk>();
        private List<LineInfo> m_LinesTextInfo = new List<LineInfo>();
        ...
        private void searchText()
        {
            foreach (LineInfo aLineInfo in m_LinesTextInfo)
            {
                ...
                else if(aLineInfo.m_Text.Length > 9) {
                    int index = m_SearchText.IndexOf(aLineInfo.m_Text);
                    if(index != -1 && index < aLineInfo.m_LineCharsList.Count)
                    {
                        TextRenderInfo aFirstLetter = aLineInfo.m_LineCharsList.ElementAt(index);
                        SearchResult aSearchResult = new SearchResult(aFirstLetter, m_PageSizeY);
                        this.m_SearchResultsList.Add(aSearchResult);
                    }
                }
        ...
        {
            ExtendedTextChunk textChunk1 = null;
            LineInfo textInfo = null;
            foreach (ExtendedTextChunk textChunk2 in this.m_DocChunks)
            {
                if (textChunk1 == null)
                {
                    textInfo = new LineInfo(textChunk2);
